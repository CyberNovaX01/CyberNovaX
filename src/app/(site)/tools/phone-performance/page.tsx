...
import { ArrowLeft, Smartphone, Cpu, Zap, Search, X, Gamepad2, Swords, TrendingUp, Calendar, Settings, ChevronRight, Flame } from "lucide-react";
...
export default function PhonePerformancePage() {
  const { lang } = useLang();
  const km = lang === "km";
    ...
    <>
      ...
      <section className="relative overflow-hidden border-b border-line">
        <div
          ...
          style={{
            background:
              "radial-gradient(ellipse at top, rgba(139,92,246,0.20) 0%, transparent 60%), linear-gradient(180deg, #0a0d14 0%, #0a0d14 100%)",
          }}
        />
        <div className="relative mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:py-14">
          ...
          <FadeIn>
            <div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.25em]" style={{ color: "#a78bfa" }}>
              <Smartphone size={13} />
              CHECK FPS YOUR PHONE
            </div>
            <h1 className="mt-4 text-3xl font-black tracking-tight sm:text-4xl lg:text-5xl">
              <span className="text-white">{km ? "ពិនិត្យ" : "Check"}</span>{" "}
              <span
                style={{
              ...
              >
                {km ? "FPS ទូរស័ព្ទ" : "Phone FPS"}
              </span>
            </h1>
            <p className="mt-3 max-w-2xl text-sm text-muted">
              {km
                ? "ពិនិត្យថាតើទូរស័ព្ទរបស់អ្នកលេង Free Fire និង MLBB បាន FPS ប៉ុន្មាន — ចុចមួយដងឃើញភ្លាម"
                : "Check your phone's FPS for Free Fire and MLBB — one click for instant results"}
            </p>
          </FadeIn>
        </div>
      </section>
        ...
        <FadeIn>
          <div className="relative">
            ...
            <input
              ...
              placeholder={km ? "ស្វែងរកទូរស័ព្ទ... (ឧ. iPhone 15, Samsung, Xiaomi)" : "Search phone... (e.g. iPhone 15, Samsung, Xiaomi)"}
      ...
      <section className="mx-auto max-w-6xl px-4 pb-24 pt-10 sm:px-6">
        <FadeIn>
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-sm font-black uppercase tracking-wider text-faint">
